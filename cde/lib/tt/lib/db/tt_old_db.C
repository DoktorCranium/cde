...
//%%  $XConsortium: tt_old_db.C /main/3 1995/10/23 10:05:40 rswiston $ 			 				
/*
 * tt_old_db.cc - Defines the old TT DB server database.
 *
...
{
  connectToDB(partition, db_conn);
}

void _Tt_old_db::connectToDB (const _Tt_string        &partition,
			      const _Tt_db_client_ptr &db_conn)
{
  dbResults = TT_DB_OK;
  dbPartition = partition;
  dbHostname = db_conn->getHostname();
  ...
  _Tt_string file_table_file(TT_OLD_DB_FILE_TABLE_FILE);
  file_table_file = base_dir.cat(file_table_file);

  _Tt_string file_object_map_file(TT_OLD_DB_FILE_OBJECT_MAP_FILE);
  file_object_map_file = base_dir.cat(file_object_map_file);

  _Tt_string property_table_file(TT_OLD_DB_PROPERTY_TABLE_FILE);
  property_table_file = base_dir.cat(property_table_file);

  _Tt_string access_table_file(TT_OLD_DB_ACCESS_TABLE_FILE);
  access_table_file = base_dir.cat(access_table_file);

  // Create key descriptor for the file key in the file table
  fileTableFileKey = new _Tt_client_isam_key_descriptor;
  fileTableFileKey->addKeyPart(TT_OLD_DB_FIRST_KEY_OFFSET,
			       TT_OLD_DB_KEY_LENGTH,
			       BINTYPE);

  fileTable = new _Tt_client_isam_file(file_table_file,
				TT_OLD_DB_KEY_LENGTH+MAXPATHLEN,
				TT_OLD_DB_KEY_LENGTH+TT_OLD_DB_MAX_KEY_LENGTH,
				fileTableFileKey,
  ...
  fileTableFilePathKey = new _Tt_client_isam_key_descriptor;
  fileTableFilePathKey->addKeyPart(TT_OLD_DB_FILE_PATH_OFFSET,
  ...
  if (!results && fileTable->isNew()) {
    ...
    (void)fileTable->writeMagicString(_Tt_string(TT_OLD_DB_VERSION));
  }
  else if (results) {
    dbResults = TT_DB_ERR_DB_OPEN_FAILED;
  }
  else {
    _Tt_string version = fileTable->readMagicString();
    if (version != _Tt_string(TT_OLD_DB_VERSION)) {
      dbResults = TT_DB_ERR_DB_OPEN_FAILED;

      _tt_syslog(0, LOG_ERR,
		 catgets(_ttcatd, 1, 6,
			 "server version (%s) does not"
			 "match the version of the database tables (%s). "
			 "Please run a version %s database server"),
		 (char *)TT_OLD_DB_VERSION, (char *)version, (char *)version);
    }
  }

  if (dbResults == TT_DB_OK) {
    // Create key descriptor for the object key in the file-object map
    fileObjectMapObjectKey = new _Tt_client_isam_key_descriptor;
    fileObjectMapObjectKey->addKeyPart(TT_OLD_DB_FIRST_KEY_OFFSET,
				       TT_OLD_DB_KEY_LENGTH,
				       BINTYPE);

    fileObjectMap = new _Tt_client_isam_file(file_object_map_file,
				      2*TT_OLD_DB_KEY_LENGTH,
				      2*TT_OLD_DB_KEY_LENGTH,
    ...
    fileObjectMapFileKey = new _Tt_client_isam_key_descriptor;
    fileObjectMapFileKey->addKeyPart(TT_OLD_DB_SECOND_KEY_OFFSET,
				     TT_OLD_DB_KEY_LENGTH,
				     BINTYPE);
    ...
    if (!results && fileObjectMap->isNew()) {
      ...
      (void)fileObjectMap->writeMagicString(_Tt_string(TT_OLD_DB_VERSION));
    }
    else if (results) {
      dbResults = TT_DB_ERR_DB_OPEN_FAILED;
    }
  ...
  if (dbResults == TT_DB_OK) {
    ...
    // the property table
    propertyTablePropertyKey = new _Tt_client_isam_key_descriptor;
    propertyTablePropertyKey->addKeyPart(TT_OLD_DB_FIRST_KEY_OFFSET,
					 TT_OLD_DB_KEY_LENGTH,
					 BINTYPE);
    propertyTablePropertyKey->addKeyPart(TT_OLD_DB_PROPERTY_NAME_OFFSET,
					 TT_OLD_DB_MAX_PROPERTY_NAME_LENGTH,
					 CHARTYPE);
    propertyTablePropertyKey->setDuplicates(TRUE);

    propertyTable =
      new _Tt_client_isam_file(property_table_file,
			ISMAXRECLEN,
			TT_OLD_DB_KEY_LENGTH+TT_OLD_DB_MAX_PROPERTY_NAME_LENGTH,
			propertyTablePropertyKey,
			ISVARLEN+ISINOUT+ISEXCLLOCK,
		        db_conn);
    propertyTableFD = propertyTable->getFileDescriptor();
    dbLastFileAccessed = propertyTable->getName();
    results = propertyTable->getErrorStatus();

    if (!results && propertyTable->isNew()) {
      (void)propertyTable->writeMagicString(_Tt_string(TT_OLD_DB_VERSION));
    }
    else if (results) {
      dbResults = TT_DB_ERR_DB_OPEN_FAILED;
    }
  ...
  if (dbResults == TT_DB_OK) {
    ...
    accessTableKey = new _Tt_client_isam_key_descriptor;
    accessTableKey->addKeyPart(TT_OLD_DB_FIRST_KEY_OFFSET,
			       TT_OLD_DB_KEY_LENGTH,
			       BINTYPE);

    accessTable = new _Tt_client_isam_file(access_table_file,
				    TT_OLD_DB_KEY_LENGTH+3*TT_OLD_DB_SHORT_SIZE,
				    TT_OLD_DB_KEY_LENGTH+3*TT_OLD_DB_SHORT_SIZE,
				    accessTableKey,
				    ISFIXLEN+ISINOUT+ISEXCLLOCK,
				    db_conn);
    ...
    if (!results && accessTable->isNew()) {
      (void)accessTable->writeMagicString(_Tt_string(TT_OLD_DB_VERSION));
    }
    else if (results) {
      dbResults = TT_DB_ERR_DB_OPEN_FAILED;
    }
...
_Tt_db_results _Tt_old_db::createFile (const _Tt_string        &file,
					     const _Tt_db_access_ptr &access)
{
  _Tt_db_key_ptr file_key = new _Tt_db_key;

  _Tt_client_isam_record_ptr record_ptr = fileTable->getEmptyRecord();
  record_ptr->setKeyPartValue(0, 0, file_key->binary());
  record_ptr->setBytes(TT_OLD_DB_FILE_PATH_OFFSET, file);
  ...
  if (!results) {
    ...
    _Tt_string file_key_bytes(TT_OLD_DB_KEY_LENGTH);
...
_Tt_db_results
_Tt_old_db::createObject (const _Tt_string        &file,
				...
				const _Tt_db_access_ptr &file_access)
{
  ...
  if (!file.len()) {
    real_file = TT_OLD_DB_FORWARD_POINTER_FILE;
  }
  else {
    real_file = file;
  }
  ...
  if (dbResults == TT_DB_OK) {
    ...
    _Tt_client_isam_record_ptr record_ptr = fileObjectMap->getEmptyRecord();
...
_Tt_db_results _Tt_old_db::removeFile (const _Tt_string        &file,
					     const _Tt_db_access_ptr &access)
{
  _Tt_string file_key;
  ...
  if (dbResults == TT_DB_OK) {
    ...
    if (dbResults == TT_DB_OK) {
      ...
      _Tt_client_isam_record_ptr key_record = fileTable->getEmptyRecord();
      key_record->setKeyPartValue(0, 0, file_key);
      long record_number = fileTable->getLastRecordNumber();
      int results = fileTable->deleteRecord(record_number, key_record);
      dbLastFileAccessed = fileTable->getName();
	...
	// Remove the file's access info
        record_number = accessTable->getLastRecordNumber();
        results = accessTable->deleteRecord(record_number, key_record);
	dbLastFileAccessed = accessTable->getName();
      }
...
_Tt_db_results _Tt_old_db::removeObject (const _Tt_string        &objid,
					       const _Tt_db_access_ptr &access)
{
  _Tt_string object_key = getObjectKey(objid);
  ...
  if (dbResults == TT_DB_OK) {
    ...
    if (!results) {
      ...
      if (!results) {
	// Delete the record
	_Tt_client_isam_record_ptr key_record =
	                           fileObjectMap->getEmptyRecord();
	key_record->setKeyPartValue(0, 0, object_key);
        long record_number = fileObjectMap->getLastRecordNumber();
        results = fileObjectMap->deleteRecord(record_number, key_record);
...
_Tt_db_results
_Tt_old_db::setFileProperty (const _Tt_string          &file,
...
_Tt_db_results
_Tt_old_db::addFileProperty (const _Tt_string          &file,
				   const _Tt_db_property_ptr &property,
				   bool_t                     unique,
				   const _Tt_db_access_ptr   &access)
{
...
_Tt_db_results
_Tt_old_db::deleteFileProperty (const _Tt_string          &file,
				      const _Tt_db_property_ptr &property,
				      const _Tt_db_access_ptr   &access)
{
  _Tt_string file_key;
...
_Tt_db_results
_Tt_old_db::deleteFileProperties (const _Tt_string        &file,
				        const _Tt_db_access_ptr &access)
{
  _Tt_string file_key;
  if (getFileKey(file, file_key) != TT_DB_OK) {
...
_Tt_db_results
_Tt_old_db::getFileProperty (const _Tt_string        &file,
...
_Tt_db_results
_Tt_old_db::getFileObjects (const _Tt_string        &file,
				  const _Tt_db_access_ptr &access,
				  _Tt_string_list_ptr     &objids)
{
...
_Tt_db_results
_Tt_old_db::deleteFileObjects (const _Tt_string        &file,
				     const _Tt_db_access_ptr &access)
{
  _Tt_string file_key;
  if (getFileKey(file, file_key) != TT_DB_OK) {
...
_Tt_db_results
_Tt_old_db::setFileFile (const _Tt_string        &file,
			       ...
			       const _Tt_db_access_ptr &access)
{
  ...
  _Tt_client_isam_record_ptr record_ptr;
  record_ptr = fileTable->readRecord(ISCURR);
  int results = fileTable->getErrorStatus();

  if (!results) {
    // Get a clean record
    _Tt_client_isam_record_ptr new_record = fileTable->getEmptyRecord();

    // Put the file key into the new record
    record_ptr->setKeyPartValue(0, 0, file_key);

    // Put the new file into the record
    record_ptr->setBytes(TT_OLD_DB_FILE_PATH_OFFSET, new_file);

    // Update the current file table record with the new info
    long record_number = fileTable->getLastRecordNumber();
    results = fileTable->updateRecord(record_number, record_ptr);
  }
...
_Tt_db_results
_Tt_old_db::getFileChildren (const _Tt_string    &file,
				   _Tt_string_list_ptr &children)
{
  ...
  if (dbResults == TT_DB_OK) {
    ...
    results =
      fileTable->findStartRecord(fileTableFilePathKey,
				 ((root_path.len() < TT_OLD_DB_MAX_KEY_LENGTH) ?
				  root_path.len() : TT_OLD_DB_MAX_KEY_LENGTH),
				 record_ptr,
				 ISEQUAL);
    dbLastFileAccessed = fileTable->getName();
  ...
  if (dbResults == TT_DB_OK) {
    ...
    while (!results) {
      ...
      if (!results) {
	_Tt_string child = record_ptr->getBytes(TT_OLD_DB_FILE_PATH_OFFSET, 0);
...
_Tt_db_results
_Tt_old_db::setFileAccess (const _Tt_string        &file,
...
_Tt_db_results
_Tt_old_db::getFileAccess (const _Tt_string        &file,
			         const _Tt_db_access_ptr &access,
...
_Tt_db_results
_Tt_old_db::setObjectProperty (const _Tt_string          &objid,
				     const _Tt_db_property_ptr &property,
				     const _Tt_db_access_ptr   &access)
{
  _Tt_string object_key = getObjectKey(objid);
...
_Tt_db_results
_Tt_old_db::addObjectProperty (const _Tt_string          &objid,
				     const _Tt_db_property_ptr &property,
				     bool_t                     unique,
				     const _Tt_db_access_ptr   &access)
{
...
_Tt_db_results
_Tt_old_db::deleteObjectProperty (const _Tt_string          &objid,
				        const _Tt_db_property_ptr &property,
				        const _Tt_db_access_ptr   &access)
{
  _Tt_string object_key = getObjectKey(objid);
...
_Tt_db_results
_Tt_old_db::deleteObjectProperties (const _Tt_string        &objid,
					  const _Tt_db_access_ptr &access)
{
  _Tt_string object_key = getObjectKey(objid);
  if (verifyObjectAccess(object_key, access, TRUE) != TT_DB_OK) {
...
_Tt_db_results
_Tt_old_db::getObjectProperty (const _Tt_string        &objid,
				     const _Tt_string        &name,
				     const _Tt_db_access_ptr &access,
				     _Tt_db_property_ptr     &property)
{
...
_Tt_db_results
_Tt_old_db::setObjectFile (const _Tt_string        &objid,
				 ...
				 const _Tt_db_access_ptr &access)
{
  ...
  if (dbResults == TT_DB_OK) {
	...
	if (dbResults == TT_DB_OK) {
	  ...
	  record_ptr->setKeyPartValue(1, 0, file_key);
    	  long record_number = fileObjectMap->getLastRecordNumber();
    	  results = fileObjectMap->updateRecord(record_number, record_ptr);
...
_Tt_db_results
_Tt_old_db::getObjectFile (const _Tt_string        &objid,
				 const _Tt_db_access_ptr &access,
				 _Tt_string              &file)
{
  file = (char *)NULL;
...
_Tt_db_results
_Tt_old_db::setObjectAccess (const _Tt_string        &objid,
				   const _Tt_db_access_ptr &new_access,
				   const _Tt_db_access_ptr &access)
{
  _Tt_string object_key = getObjectKey(objid);
...
_Tt_db_results
_Tt_old_db::getObjectAccess (const _Tt_string        &objid,
				   const _Tt_db_access_ptr &access,
				   _Tt_db_access_ptr       &current_access)
{
  current_access = (_Tt_db_access *)NULL;
...
_Tt_db_results
_Tt_old_db::verifyObjectAccess (const _Tt_string        &object_key,
				      const _Tt_db_access_ptr &access,
				      bool_t                   write,
				      bool_t                   user_only)
{
...
_Tt_db_results
_Tt_old_db::verifyAccess (const _Tt_string        &key,
				const _Tt_db_access_ptr &access,
				bool_t                   write,
				bool_t                   user_only)
{
  _Tt_client_isam_record_ptr record_ptr = accessTable->getEmptyRecord();
    ...
    if (results) {
      ...
      short n_user = *(short *)
		      ((char *)record_ptr->getRecord()+
			       TT_OLD_DB_ACCESS_USER_OFFSET);
      uid_t user = (uid_t)ntohs(n_user);

      short n_group = *(short *)
		       ((char *)record_ptr->getRecord()+
			        TT_OLD_DB_ACCESS_GROUP_OFFSET);
      gid_t group = (gid_t)ntohs(n_group);
...
_Tt_db_results _Tt_old_db::getFileKey (const _Tt_string &file,
					     _Tt_string &file_key)
{
  dbResults = TT_DB_OK;
  ...
  int results =
    fileTable->findStartRecord(fileTableFilePathKey,
			       ((file.len() < TT_OLD_DB_MAX_KEY_LENGTH) ?
				file.len() : TT_OLD_DB_MAX_KEY_LENGTH),
			       record_ptr,
			       ISEQUAL);
  dbLastFileAccessed = fileTable->getName();
  ...
  if (results == ENOREC) {
	...
	_Tt_string temp_file = (char *)
                               record_ptr->getBytes(TT_OLD_DB_FILE_PATH_OFFSET, 0);
...
_Tt_string _Tt_old_db::getObjectKey (const _Tt_string &objid)
{
  ...
  _Tt_string object_key_bytes(TT_OLD_DB_KEY_LENGTH);
...
_Tt_db_results _Tt_old_db::addPropertyValue(const _Tt_string &key,
						  const _Tt_string &name,
						  const _Tt_string &value)
{
  dbResults = TT_DB_OK;

  _Tt_client_isam_record_ptr record_ptr = propertyTable->getEmptyRecord();
  record_ptr->setKeyPartValue(0, 0, key);
  record_ptr->setKeyPartValue(0, 1, name);

  record_ptr->setBytes(TT_OLD_DB_PROPERTY_VALUE_OFFSET, value);
  record_ptr->setLength(TT_OLD_DB_PROPERTY_VALUE_OFFSET+value.len());
  ...
  else if (results == (int)TT_OLD_DB_ACCESS_DENIED) {
    dbResults = TT_DB_ERR_ACCESS_DENIED;
  }
  else if (results) {
    dbResults = TT_DB_ERR_CORRUPT_DB;
...
_Tt_db_results
_Tt_old_db::setProperty (const _Tt_string          &key,
			       const _Tt_db_property_ptr &property)
{
...
_Tt_db_results
_Tt_old_db::setProperties (const _Tt_string               &key,
				 const _Tt_db_property_list_ptr &properties)
{
...
_Tt_db_results
_Tt_old_db::addProperty (const _Tt_string          &key,
			       const _Tt_db_property_ptr &property,
			       bool_t                     unique)
{
  dbResults = TT_DB_OK;
  ...
  while (values_cursor.next() && (dbResults == TT_DB_OK)) {
	...
	for (;;) {
	  // Read the next record
	  results = propertyTable->readRecord(ISNEXT, record_ptr);
	  ...
	  if (!results) {
	    ...
	    temp_string = record_ptr->getBytes(TT_OLD_DB_PROPERTY_VALUE_OFFSET, 0);
...
_Tt_db_results
_Tt_old_db::deleteProperty (const _Tt_string          &key,
				  const _Tt_db_property_ptr &property)
{
  dbResults = TT_DB_OK;
  ...
  if (results == ENOREC) {
	...
	if (property->is_empty()) {
	  found = TRUE;
          long record_number = propertyTable->getLastRecordNumber();
          results = propertyTable->deleteRecord(record_number, record_ptr);
          if (results == (int)TT_OLD_DB_ACCESS_DENIED) {
            dbResults = TT_DB_ERR_ACCESS_DENIED;
	    break;
          }
	  else if (results) {
	...
	// Else specific values to delete have been specified...
	  ...
	  _Tt_string property_value =
	    record_ptr->getBytes(TT_OLD_DB_PROPERTY_VALUE_OFFSET, 0);
		    ...
		    bool_t            preserve_special_properties)
{
  ...
  for (;;) {
	...
	if (preserve_special_properties) {
	  ...
	  if (strcmp((char *)temp_string, TT_OLD_DB_PROPS_CACHE_LEVEL_PROPERTY) &&
	      strcmp((char *)temp_string, TT_OLD_DB_OBJECT_TYPE_PROPERTY)) {
            long record_number = propertyTable->getLastRecordNumber();
            results = propertyTable->deleteRecord(record_number, record_ptr);
            if (results) {
              dbResults = TT_DB_ERR_CORRUPT_DB;
...
_Tt_db_results
_Tt_old_db::getProperty (const _Tt_string    &key,
			       const _Tt_string    &name,
			       _Tt_db_property_ptr &property)
{
  dbResults = TT_DB_OK;
...
_Tt_db_results
_Tt_old_db::getProperties (const _Tt_string         &key,
				 _Tt_db_property_list_ptr &properties)
{
  dbResults = TT_DB_OK;
  ...
  if (results && (results != ENOREC)) {
	...
	value = record_ptr->getBytes(TT_OLD_DB_PROPERTY_VALUE_OFFSET, 0);
...
_Tt_db_results _Tt_old_db::setAccess (const _Tt_string        &key,
					    const _Tt_db_access_ptr &access)
{
  dbResults = TT_DB_OK;
  ...
  if (results == ENOREC) {
    *(short *)
     ((char *)record_ptr->getRecord()+
	      TT_OLD_DB_ACCESS_USER_OFFSET) = htons((short)access->user);
    *(short *)
     ((char *)record_ptr->getRecord()+
              TT_OLD_DB_ACCESS_GROUP_OFFSET) = htons((short)access->group);
    *(short *)
     ((char *)record_ptr->getRecord()+
              TT_OLD_DB_ACCESS_MODE_OFFSET) = htons((short)access->mode);
    ...
    if (results) {
      ...
      *(short *)
      ...
      long record_number = accessTable->getLastRecordNumber();
      results = accessTable->updateRecord(record_number, record_ptr);
...
_Tt_db_results _Tt_old_db::getAccess (const _Tt_string  &key,
  ...
  if (results) {
    ...
    if (results) {
      ...
      short n_user = *(short *)
	              ((char *)record_ptr->getRecord()+
		               TT_OLD_DB_ACCESS_USER_OFFSET);
      access->user = (uid_t)ntohs(n_user);
      ...
      short n_mode = *(short *)
		      ((char *)record_ptr->getRecord()+
		               TT_OLD_DB_ACCESS_MODE_OFFSET);
      access->mode = (mode_t)ntohs(n_mode);
...
_Tt_db_results _Tt_old_db::getFile (const _Tt_string &key,
				    _Tt_string       &file) 
{ 
  _Tt_client_isam_record_ptr record_ptr = fileObjectMap->getEmptyRecord();
  record_ptr->setKeyPartValue(0, 0, key);
    ...
    if (!results) {
      ...
      if (results) {
        ...
        if (results) {    
          ...
          file = record_ptr->getBytes(TT_OLD_DB_FILE_PATH_OFFSET, 0);
          dbResults = TT_DB_OK;
        }
      } 
    }    
